...
const VendorManagement = () => {
    ...
    const fetchVendorTickets = async () => {
        try {
            setLoading(true);
            const response = await apiService.getAllVendors('approved');
            setTickets(response.success ? response.vendors : []);
        } catch (err) {
                                    ...
                                    <tr key={ticket._id} className="hover:bg-white/40 transition-colors group">
                                        ...
                                        <div className="flex items-center justify-end gap-2">
                                            <button
                                                ...
                                                className="p-2 hover:bg-gray-100 rounded-full transition-colors text-gray-400 hover:text-gray-900 group/reply relative"
                                            ...
                                            <button
                                                onClick={async () => {
                                                    if (window.confirm("Are you sure you want to permanently delete this vendor? This action cannot be undone.")) {
                                                        try {
                                                            await apiService.deleteVendor(ticket.userId?._id || ticket._id);
                                                ...
                                                className="p-2 hover:bg-red-50 rounded-full transition-colors text-gray-400 hover:text-red-600 group/delete relative"
                                                title="Delete Vendor"
                                            >
